...
impl OsuPPInner {
    ...
    fn compute_accuracy_value(&self) -> f64 {
        ...
        // For relax increase acc_value as percentage grows instead of completely zeroing it out
        if self.mods.rx() {
    ...
    fn compute_flashlight_value(&self) -> f64 {
        ...
        } else if self.mods.hd() && self.mods.hr() {
            flashlight_value *= 1.25; // Reduce bonus for HD when HR is also active (nerfs 4-mod scores)
        }

        // Nerf flashlight value for relax and autopilot
